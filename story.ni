...
Stating is an action applying to one visible thing. Understand "[any seen thing]" as stating.
Check stating:
...
Instead of drinking the stream, say [M73]"You have taken a drink from the stream.  The water tastes strongly of minerals, but is not unpleasant.  It is extremely cold."
...
[O2]A brass lantern is a device in Inside_Building. [D0+1]"There is a [if switched off]shiny brass lamp [otherwise]lamp shining [end if]nearby."
Understand "lamp/headlamp/headlight/light/shiny" as the brass lantern.
Every turn when the lantern is switched on:
...
Instead of examining the lantern, say [ccr]"It is a shiny brass lamp[if switched off].  It is not currently lit.[otherwise if power remaining of lantern is less than 30], glowing dimly.[otherwise], glowing brightly."
...
Instead of traveling to Outside_Grate when the location is In_Cobble_Crawl or the location is In_Debris_Room or the location is In_Awkward_Sloping_E_W_Canyon or the location is In_Bird_Chamber or the location is At_Top_Of_Small_Pit:
	if the steel grate is locked, move player to Below_The_Grate as going;
	otherwise continue the action.
...
[ccr]A dome is climbable scenery in In_Hall_Of_Mists.
Instead of examining the dome:
	if the gold nugget is enclosed by the player, say "I'm not sure you'll be able to get up it with what you're carrying.";
	otherwise say "It looks like you might be able to climb up it."
...
Rule for writing a paragraph about the beanstalk (this is the describe beanstalk rule):
	if height of plant is tall, say [D1]"The top of a 12-foot-tall beanstalk is poking out of the west pit." (A);
	otherwise say [D2]"There is a huge beanstalk growing out of the west pit up to the hole." (B).
Instead of examining the beanstalk, follow the describe beanstalk rule.
...
Instead of pouring the bottled water on the plant:
	...
	say line break;
	describe the plant.
Instead of examining the plant, describe the plant.
...
Instead of reading the sign, say "It says, 'Cave under construction beyond this point.  Proceed at own risk.  [bracket]Witt Construction Company[close bracket]'".
...
[O63]Some rare spices are a treasure in In_Chamber_Of_Boulders with deposit points 14. [D0]"There are rare spices here!"
Understand "spice/exotic" as the rare spices.
Instead of smelling or examining the rare spices, say [ccr]"They smell wonderfully exotic!"
...
Instead of asking the bear to try doing something, say "This is a Bear of very little brain."
Instead of asking the bear about something, say "This is a Bear of very little brain."
Instead of answering the bear that something, say "This is a Bear of very little brain."
...
After going when the bear is following you:
	...
	continue the action.
Rule for choosing notable locale objects for In_Barren_Room (this is the put chain after bear rule):
	if the chain is locked:
		set the locale priority of the bear to 3;
